...
export default function StatisticsPage() {
  ...
  const isAiDerivedCategory = category === "ai_parity" || category === "ai_size";
  const isPairCategory = category === "ai" || isAiDerivedCategory || category === "off_digit" || category === "off_jumlah" || category === "off_shio";
  const selectedBBFS = bbfsScopeMeta(bbfsScope);
  const effectiveTargetPair = isBBFSCategory ? selectedBBFS.targetPair : targetPair;
  const paramOptions = category === "ai" ? [2, 4, 6] : category === "bbfs" ? [7, 8, 9] : isAiDerivedCategory ? [1] : [1, 2, 3];
  const currentFilterLabel = isBBFSCategory ? `${categoryMeta.title} ${selectedBBFS.label} ${param}` : isPositionCategory ? `2D ${targetPairLabel(targetPair)} OFF ${param}` : isAiDerivedCategory ? `${categoryMeta.title} ${targetPairLabel(targetPair)}` : `${categoryMeta.title} ${targetPairLabel(targetPair)} ${category === "ai" ? `${param}D` : `OFF ${param}`}`;
  ...
  const loadStatistics = async () => {
    ...
    try {
      ...
      if (!marketIds.length) return setRelatedStats({});
      ...
      setRelatedStats(cleanRelated.reduce<RelatedStatsMap>((acc, row) => {
  ...
  useEffect(() => {
    ...
    if (isAiDerivedCategory && param !== 1) setParam(1);
    if (!["ai", "ai_parity", "ai_size", "bbfs"].includes(category) && ![1, 2, 3].includes(param)) setParam(1);
  }, [category]);
  ...
  return (
    <div className="statistics-page fixed inset-0 z-50 overflow-y-auto animate-[riseIn_0.35s_ease-out] px-4 py-4 pb-[calc(2rem+env(safe-area-inset-bottom))] sm:px-6" style={{ color: "var(--text)", background: "radial-gradient(circle at 10% 0%, rgba(52,211,153,0.16), transparent 28%), radial-gradient(circle at 90% 18%, rgba(246,201,107,0.12), transparent 26%), radial-gradient(circle at 50% 80%, rgba(99,102,241,0.12), transparent 32%), linear-gradient(180deg,#03070a 0%,#06100e 46%,#030407 100%)" }}>
      <div className="mx-auto w-full max-w-3xl space-y-5">
        <button onClick={() => navigate("/")} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-[11px] font-black uppercase tracking-[2px] text-[var(--text-dim)] shadow-lg active:scale-95"><ArrowLeft size={17} /> Beranda</button>
        ...
        <section>
          <div className="relative overflow-hidden rounded-[2rem] border border-emerald-300/18 bg-[linear-gradient(135deg,rgba(8,18,18,0.96),rgba(13,11,22,0.96))] p-5 shadow-[0_22px_54px_rgba(0,0,0,0.42)]">
            ...
            <div className="relative flex items-start justify-between gap-3">
              ...
              <button onClick={loadStatistics} className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-white/12 bg-white/[0.055] text-[var(--text-dim)] shadow-inner active:scale-95" aria-label="Refresh statistik"><RefreshCw size={19} className={loading ? "animate-spin" : ""} /></button>
            </div>
            <div className="relative mt-5 grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-emerald-300/12 bg-black/35 px-4 py-3"><p className="text-[8px] font-black uppercase tracking-[1.5px] text-[var(--text-dim)]">Mode</p><p className="mt-1 truncate font-['Orbitron'] text-[12px] font-black uppercase tracking-[1.8px]" style={{ color: statAccent }}>{currentFilterLabel}</p></div>
              <div className="rounded-2xl border border-amber-200/12 bg-black/35 px-4 py-3"><p className="text-[8px] font-black uppercase tracking-[1.5px] text-[var(--text-dim)]">Update</p><p className="mt-1 truncate font-['Orbitron'] text-[11px] font-black uppercase tracking-[1.4px]" style={{ color: statGold }}>{formatUpdatedAt(latestUpdate)}</p></div>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <Panel className="space-y-4 p-4">
            <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/[0.08] bg-black/25 px-3 py-2"><p className="text-[9px] font-black uppercase tracking-[1.8px] text-[var(--text-dim)]">{isBBFSCategory ? "Scope BBFS" : isPositionCategory ? "2D Posisi" : "Fokus"}</p><span className="max-w-[62%] truncate rounded-full px-3 py-1.5 text-[9px] font-black uppercase tracking-[1px] text-black" style={{ background: statGold }}>{currentFilterLabel}</span></div>
            {isBBFSCategory ? (
              <div className="space-y-3 rounded-[1.35rem] border border-white/[0.08] bg-black/20 p-3"><div className="grid grid-cols-2 gap-2">{bbfsScopes.map((item) => {
              ...
              })}</div><p className="rounded-2xl border border-white/[0.06] bg-black/30 px-3 py-2.5 text-center text-[9px] font-black uppercase tracking-[1.4px] text-[var(--text-dim)]">{selectedBBFS.subtitle}</p></div>
            ) : (
              <div className="space-y-3 rounded-[1.35rem] border border-white/[0.08] bg-black/20 p-3"><div className="grid grid-cols-3 gap-2">{targetPairs.map((item) => {
              ...
              })}</div>{isPositionCategory && <p className="rounded-2xl border border-white/[0.06] bg-black/30 px-3 py-2.5 text-center text-[9px] font-black uppercase tracking-[1.4px] text-[var(--text-dim)]">{positionPairSubtitle(targetPair)}</p>}</div>
            )}
            <div className="rounded-[1.35rem] border border-amber-200/[0.10] bg-black/20 p-3"><p className="mb-2 text-[9px] font-black uppercase tracking-[1.8px] text-[var(--text-dim)]">Parameter</p><div className="grid grid-cols-3 gap-2">{paramOptions.map((value) => {
              const active = param === value;
              const label = category === "ai" ? `${value}D` : isAiDerivedCategory ? categoryMeta.title : String(value);
              return <button key={value} type="button" onClick={() => setParam(value)} className={`${isAiDerivedCategory ? "col-span-3" : ""} min-h-[54px] rounded-[1.05rem] px-2 py-3 text-[10px] font-black uppercase tracking-[1.2px] transition active:scale-[0.985]`} style={{ background: active ? "linear-gradient(135deg,#f6c96b,#facc15)" : "rgba(255,255,255,0.045)", color: active ? "#120d02" : "var(--text-dim)", border: active ? "1px solid rgba(254,240,138,0.75)" : "1px solid rgba(255,255,255,0.075)", boxShadow: active ? "0 12px 28px rgba(246,201,107,0.18)" : "none" }}>{label}</button>;
            })}</div></div>
          </Panel>
        </section>
        ...
        <section>
          ...
          {loading ? <Panel className="p-6 text-center"><div className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border-4 border-white/10" style={{ borderTopColor: statAccent }} /><p className="font-['Orbitron'] text-[12px] font-black uppercase tracking-[2px] text-[var(--text)]">Memuat Ranking</p><p className="mt-2 text-[12px] leading-5 text-[var(--text-dim)]">Mengambil statistik pasaran terbaru.</p></Panel> : topItems.length ? <div className="grid gap-3">{topItems.map((item, index) => <StatisticCard key={item.id || `${item.market_id}-${item.group_key}-${item.param}-${item.position}-${item.target_pair}-${item.analysis_scope}`} item={item} index={index} relatedStats={relatedStats} onOpen={navigate} />)}</div> : <Panel className="p-7 text-center"><div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-black/30 text-[var(--text-dim)]"><BarChart3 /></div><p className="font-['Orbitron'] text-[14px] font-black uppercase tracking-[2px] text-[var(--text)]">Belum ada ranking</p><p className="mx-auto mt-3 max-w-sm text-[12px] leading-5 text-[var(--text-dim)]">{error ? "Statistik belum bisa dimuat. Pastikan evaluator statistik sudah berjalan." : `Belum ada pasaran yang masuk kriteria ${currentFilterLabel}.`}</p><button onClick={loadStatistics} className="mt-5 rounded-2xl border border-white/10 bg-white/[0.06] px-5 py-3 text-[11px] font-black uppercase tracking-[1.5px] text-[var(--text)] active:scale-[0.985]">Muat Ulang</button></Panel>}
        </section>
      </div>
    </div>
  );
