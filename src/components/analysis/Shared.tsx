...
export function MiniLabel({ children }: { children: React.ReactNode }) {
  return <div className="text-[9px] font-black uppercase tracking-[2px] text-[var(--text-dim)]">{children}</div>;
}
...
export function DetailToggle({ open, accent, onClick }: { open: boolean; accent: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[9px] font-black uppercase tracking-[1px] active:scale-95" style={{ borderColor: `${accent}55`, backgroundColor: `${accent}18`, color: accent }} aria-label={open ? "Tutup" : "Buka"}>{open ? <ChevronUp size={13} /> : <ChevronDown size={13} />}{open ? "Tutup" : "Buka"}</button>;
}
...
export function LineBox({ label, lines, accent, soft }: { label: string; lines: string[]; accent: string; soft: string }) {
  ...
  return <div key={label} className="rounded-3xl border border-[var(--border2)] bg-black/20 p-4"><div className="mb-3 flex items-center justify-between gap-3"><span className="font-['Orbitron'] text-[11px] font-black uppercase tracking-[2px] text-[var(--text)]">{label}</span><span className="rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[1px]" style={{ backgroundColor: soft, color: accent }}>{lines.length} LINE</span></div><div className="max-h-[260px] overflow-y-auto rounded-3xl border border-[var(--border2)] bg-black/30 p-4 font-['JetBrains_Mono'] text-[14px] font-bold leading-8 tracking-[2px] custom-scrollbar" style={{ color: accent }}>{displayPayload || "-"}</div><button onClick={() => navigator.clipboard?.writeText(copyPayload)} className="mt-3 flex w-full items-center justify-center gap-2 rounded-3xl p-4 font-['Orbitron'] text-[11px] font-black uppercase tracking-[3px] text-black transition active:scale-95" style={{ backgroundColor: accent }}><Copy size={16} /> Copy Semua</button></div>;
}
