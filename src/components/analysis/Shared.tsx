...
export function LineBox({ label, lines, accent, soft }: { label: string; lines: string[]; accent: string; soft: string }) {
  ...
  return <div key={label} className="ui-card ui-motion-in rounded-3xl p-4"><div className="mb-3 flex items-center justify-between gap-3"><span className="ui-title text-[11px]">{label}</span><span className="rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[1px]" style={{ backgroundColor: soft, color: accent }}>{lines.length} LINE</span></div><div className="max-h-[260px] overflow-y-auto rounded-3xl border border-[var(--ui-border-soft)] bg-black/30 p-4 font-['JetBrains_Mono'] text-[14px] font-bold leading-8 tracking-[2px] custom-scrollbar" style={{ color: accent }}>{displayPayload || "-"}</div><button onClick={() => navigator.clipboard?.writeText(copyPayload)} className="ui-motion-soft ui-tap mt-3 flex w-full items-center justify-center gap-2 rounded-3xl p-4 font-['Orbitron'] text-[11px] font-black uppercase tracking-[3px] text-black" style={{ backgroundColor: accent }}><Copy size={16} /> Copy Semua</button></div>;
}