    ...
    <div className="premium-panel mt-4 p-4">
      ...
      <div className={`grid gap-2 ${isThreeDigitMode || isAiMode ? "grid-cols-3" : "grid-cols-2 sm:grid-cols-4"}`}>
        {cfg.values.map((n: number) => {
          ...
          return (
            <button key={n} onClick={() => onAnalyze(n)} className={`${isWideBBFS ? "col-span-3" : ""} rounded-3xl border ${isThreeDigitMode || isAiMode ? "min-h-[92px] p-3" : "p-5"} text-center transition active:scale-95`} style={{ borderColor: meta.accent, backgroundColor: meta.soft, color: meta.accent }}>
