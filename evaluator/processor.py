...
from .config import MATI_POSITIONS, MODES, TARGET_PAIRS
...
def save_evaluation(market_id, market_name, mode, param, target_pair, snapshot, new_result):
    ...
    if mode == "mati":
        ...
        return saved
    is_hit, target, status, detail = evaluate_snapshot(mode, param, snapshot_result, new_result, target_pair)
...
def process_market(market):
    ...
    for mode, params in MODES.items():
        for target_pair in get_mode_target_pairs(mode):
            for param in params:
                snapshot = get_one("analysis_snapshots", market_id=market_id, mode=mode, param=param, target_pair=target_pair)
                try:
                    payload, result = analyze_mode(history, mode, param, target_pair)
                except Exception as error:
                    print(f"ANALYSIS ERROR: {market_id} {mode} {param} {target_pair} {error}")
                    continue
                if snapshot and snapshot.get("base_result") != latest_result:
                    save_evaluation(market_id, market_name, mode, param, target_pair, snapshot, latest_result)
                if not snapshot or snapshot.get("base_result") != latest_result:
                    save_snapshot(market_id, market_name, mode, param, target_pair, latest_result, result, payload)
                    print(f"ANALYSIS SNAPSHOT OK: {market_id} {mode} {param} {target_pair} base={latest_result}")
                else:
                    print(f"ANALYSIS NO CHANGE: {market_id} {mode} {param} {target_pair} result masih {latest_result}")
    return True
