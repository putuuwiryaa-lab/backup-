...
function _0xEngineAI(D: string[]) {
  ...
  return sorted.slice(0, 6).map(x => x.d).sort((a, b) => a - b);
}
...
export function runAnalysis(type: string, payload: string[], param: number) {
  ...
  if (type === 'rekap') {
    ...
    const CT = isTop ? _0xEngineAI(D).slice(0, 4) : _0xEngineAI(D).slice(0, 6);
  ...
  if (type === 'ai') {
    ...
    const sorted = Object.keys(vote).map(k => ({ d: parseInt(k), v: vote[parseInt(k)], g: gap[parseInt(k)] }));
    sorted.sort((a, b) => { if (b.v !== a.v) return b.v - a.v; return b.g - a.g; });
    const aiResult = sorted.slice(0, param).map(x => x.d).sort((a, b) => a - b);
