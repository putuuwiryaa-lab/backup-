...
app.get("/api/markets", async (req, res) => {
  ...
  try {
    ...
    console.log(`[VERCEL API] Berhasil menarik ${markets.length} pasaran dari Firebase SDK.`);
    res.json(markets);
  } catch (e: any) {
    console.error("[VERCEL API] FIREBASE SDK ERROR:", e.message);
...
app.post("/api/markets/update", async (req, res) => {
  ...
  try {
    ...
    console.error("[VERCEL API] PATCH ERROR", e.message);
